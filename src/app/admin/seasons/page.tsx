            ...
            <div key={s.id} className="flex items-center justify-between px-5 py-4">
              <div>
                <div className="font-bold text-sm flex items-center gap-2 text-zinc-900 dark:text-white">
                  <SportIcon slug={s.sport?.slug} className="h-4 w-4 text-vanguard-volt" /> {s.name}
                  {s.is_active && (
              ...
              <Link href={`/admin/seasons/${s.id}`} className="text-sm font-semibold text-vanguard-volt dark:text-vanguard-volt hover:underline">
                Edit
              </Link>
            </div>
          ))}
